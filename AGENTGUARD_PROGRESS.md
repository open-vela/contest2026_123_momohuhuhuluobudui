...
## 2026-09-14 双脸计数稳定与多框显示验收

使用“一名真人 + 屏幕正面人脸照片”构造两脸场景，不将其表述为双真人或活体检测。
修复前 ESP-DL 已能令界面到达 `FACE:2`，但会在 1/2 间变化；根因之一是通用
`face_presence` 只保持正数到 0 的变化，任意正数结果都会立即覆盖较高计数。将保持
语义收紧为：更高或相同的 ESP-DL 计数立即更新，较低计数在现有 1.5 秒窗口内沿用
最近一次较高计数，持续超过窗口后正常降级。测试先在 `2 -> 1` 处失败，再验证窗口
内保持、窗口边界、超时降级和时钟回退行为。

修复计数后实机基本稳定为 2，但 `FACE:2` 仍只有一个框。数据流检查确认官方 detector
确实返回两项，丢失发生在 AgentGuard 兼容层：`ag_vision_result` 只携带最大面积的
`primary_face`，显示 worker 也只传递和绘制该框。结果结构现保留最多 8 个有效检测框，
`primary_face` 继续仅供姿态评分；ESP-DL 算法、模型和阈值未修改。保持层同步缓存对应
框数组，LCD worker 映射并逐一绘制所有有效框。启发式后端和 fallback 同步填充统一
接口，但 ESP-DL 产品构建仍不调用肤色 fallback。

TDD 覆盖两检测框输出、`FACE:2` 保持时两个框不丢失、超时降到一框，以及两个框独立
映射到 LCD 预览区域。完整 AgentGuard 主机测试通过，ESP32-S3 交叉编译成功；烧录
报告 `Hash of data verified`。烧录后的镜像为 2,414,612 bytes，SHA-256
`35a792febe4a1451fe15314a15945b96e30325217d50cd7610461f05388a606e`。最终用户实机
确认两脸场景计数和双框显示正常。该验收只证明普通二维人脸检测与人数/框传递，不具备
活体判断或主人身份识别能力。

**Decision: `SHIP_ESP_DL_MULTI_FACE_BOXES`。** 当前容易交付的 0/1/2+ 检测、计数稳定
和多框显示已形成闭环；主人识别继续延期。
## 2026-09-17：久坐/隐私指示灯与 BOOT 多手势实机验收

- 新增独立板载绿色 LED 工作线程：久坐/待确认使用慢闪，隐私遮罩使用双闪，避免同步写灯阻塞摄像头与 LCD 主循环。
- 新增手动隐私开关并复用既有多人检测：隐私开启时显示 `PRIVACY READY`，检测到两人及以上时显示 `PRIVACY SHIELD`，并继续向 PC 代理发送遮罩事件。
- ESP32-S3-EYE 的四个 ADC 功能键在当前 NuttX 运行时采样固定为约 1331 mV，按下 MENU/UP+ 不变化；未继续用猜测阈值冒充可用输入，产品交互回退到已实机验证的 GPIO0 BOOT 键。
- BOOT 在松开时按持续时间分类：短按确认提醒，3–7 秒切换隐私模式，8 秒以上切换 Demo 模式。用户已实机确认隐私和 Demo 均可切换。
- 完整 AgentGuard 主机测试通过，ESP32-S3 镜像成功生成并写入 0x0；最终刷写 SHA-256 为 `fc0058aec731a8b85b0e2278ec2b54fe5298d4417bf3a45c925cc491c7e83a64`，esptool 报告 `Hash of data verified`。未擦除或覆盖 `0x300000` 起始的数据分区。
