...
#elif CONFIG_TIE728_BOOST && !CONFIG_AGENTGUARD_ESP_DL_FORCE_C_DEPTHWISE
    if (args.output_channel % 8 == 0 && args.input_channel % 8 == 0 && !((unsigned)&args.input_element[0] & 15) &&
        !((unsigned)&args.output_element[0] & 15)) {
        if (args.bias_element) {
            switch (args.activation_type) {
                                                       ...
                                                       const ArgsType<int8_t> &args)
{
    ...
    if (args.bias_element) {
        ...
        case ReLU:
            ...
            break;
        case LeakyReLU:
            // n_wise_func = buffer_bias_leakyrelu<int8_t, int32_t, int32_t>;
            break;
        case PReLU:
            // n_wise_func = buffer_bias_prelu<int8_t, int32_t, int32_t>;
            break;
        }
    } else {
        switch (args.activation_type) {
