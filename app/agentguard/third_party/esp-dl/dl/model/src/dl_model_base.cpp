...
/* AgentGuard NuttX port: this translation unit is rebuilt when the parser ABI
 * adapter changes, because it instantiates all operation deserializers. */
...
{
    m_fbs_loader = new fbs::FbsLoader(name, location);
    fbs::FbsModel *loaded_model = m_fbs_loader->load(model_name, key, param_copy);
    return this->load(loaded_model);
}
...
static void print_memory_info(const std::map<std::string, mem_info_t> &info)
{
    ...
    auto get_fmt_size = [&sub_prefix](size_t size, bool sub_header) -> std::string {
        char buffer[32];
        std::snprintf(buffer, sizeof(buffer), "%.2fKB", size / 1024.f);
        std::string fmt_size(buffer);
        if (sub_header) {
    ...
    for (int i = 0; i < row_headers.size(); i++) {
        ...
        std::string label = sub_header ? (sub_prefix + row_header) : row_header;
        ...
        ESP_LOGI(TAG,
                 "| %-*s | %-*s | %-*s | %-*s |",
                 static_cast<int>(col0_width),
                 label.c_str(),
                 static_cast<int>(col1_width),
                 internal.c_str(),
                 static_cast<int>(col2_width),
                 psram.c_str(),
                 static_cast<int>(col3_width),
                 flash.c_str());
