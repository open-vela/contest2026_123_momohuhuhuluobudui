...
public:
    ...
    {
        // NuttX's pthread mutex layout is not ABI-compatible with the
        // libstdc++ runtime's guarded-local recursive mutex on this target.
        // AgentGuard deliberately runs ESP-DL on one core, so a constant-
        // initialized pointer avoids __cxa_guard_acquire without losing the
        // required serialization.
        static ModuleCreator *instance = nullptr;
        if (instance == nullptr) {
            instance = new ModuleCreator();
        }
        return instance;
    }
    /**
