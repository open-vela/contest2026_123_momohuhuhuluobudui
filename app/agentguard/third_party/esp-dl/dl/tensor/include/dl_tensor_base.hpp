...
std::string vector_to_string(std::vector<T> shape)
{
    ...
    for (int i = 0; i < shape.size(); i++) {
        ...
        if constexpr (std::is_floating_point_v<T>) {
            snprintf(value, sizeof(value), "%g", static_cast<double>(shape[i]));
        } else if constexpr (std::is_signed_v<T>) {
            snprintf(value, sizeof(value), "%lld", static_cast<long long>(shape[i]));
        } else {
            snprintf(value, sizeof(value), "%llu", static_cast<unsigned long long>(shape[i]));
        }
        str += value;
        if (i != shape.size() - 1) {
            str += ", ";
...
public:
    ...
    TensorBase(std::vector<int> shape,
               ...
               bool deep = true,
               unsigned long caps = MALLOC_CAP_DEFAULT);
