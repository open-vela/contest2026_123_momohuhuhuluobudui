...
public:
     ...
     */
    /* Espressif's ESP32-S3 parser archive is built with unsigned long for
     * heap capability arguments.  Keep that ABI when NuttX's uint32_t is
     * instead an unsigned int (both are 32-bit on Xtensa). */
