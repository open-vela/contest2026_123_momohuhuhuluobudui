...
{
    (void)key;
    ESP_LOGE(TAG, "encrypted ESP-DL models are not supported by the NuttX adapter");
                                   ...
                                   uint32_t &offset)
{
    if (model_location != MODEL_LOCATION_IN_SDCARD) {
        ...
        for (int i = 0; i < model_num; i++) {
            ...
            if (name_length == requested_length &&
                memcmp(fbs_buf + name_offset, name, name_length) == 0) {
                offset = header[2 + 3 * i];
                return ESP_OK;
            }
        }
                           ...
                           bool param_copy)
{
    ...
    FbsModel *model = new FbsModel(model_buf, size, model_location, mode, rodata_move, auto_free, param_copy);
    return model;
}
...
FbsModel *FbsLoader::load(const char *model_name, const uint8_t *key, bool param_copy)
{
    ...
    FbsModel *model = create_fbs_model((const char *)m_fbs_buf, format, m_location, offset, key, param_copy);
    return model;
}
